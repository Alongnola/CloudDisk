...
<div style="font-size: 30px ; text-align: center"; >我的云盘iCloud
    <div style="font-size: 20px; text-decoration: underline">${user_name}</div>
</div>

<br/>

<form action="${pageContext.request.contextPath}/upload" method="post" enctype="multipart/form-data">
    <input type="submit" onclick="return checkfile()" value="上传文件"/>
    <input type="file" onchange="checkfile()" id="fileupload" name="file"
    ...
    ${message }
    <div id="box"></div>
    <!-- <button id="cal" type="button" onclick="calculate()">点击检查是否支持急速上传</button> -->
</form>
<br/>
...
<div style="text-align: center">
    ...
    <br/>
    <div style="font-size: 20px ;"><font> 我的文件列表 </font></div>
        ...
        <tr>
            ...
            <c:forEach var="c" items="${requestScope.pagebean.list}" varStatus="stat">
                <tr>
                    ...
                    <td>${c.createtime }</td>
                    <td>
                        <a href="${pageContext.request.contextPath}/download?id=${c.id }&filename=${c.filename }">下载</a>
                    </td>
                    <td>
                        <form>
                            <select id="${c.id}"
    ...
    <script type="text/javascript">
        ...
        function godelete(currentpage, fileid) {
            ...
            if (r == true) {
                window.location.href = '${pageContext.request.contextPath}/deleteFile?currentpage=' + currentpage + '&id=' + fileid;
            } else {
                return false;
            }
        }
        ...
        function gochange(currentpage, fileid) {
           ...
           // var pagesize = document.getElementById("pagesize").value;
            ...
            if (r == true) {
                window.location.href = '${pageContext.request.contextPath}/Share?currentpage=' + currentpage + '&id=' + fileid + '&canshare=' + canshare;
            } else {
                location.reload();
            }
        }
                    ...
                    $.ajax({
                        ...
                        success: function (res) {  //res是server端响应
                            ...
                            if (response.fast == "true") {
                                ...
                                $("#cal").click(function(){
                                    $.ajax({
                                        url: "https://127.0.0.1:8080/FastUpload",
                                        type: "POST",
                                        data: {
                                            "MD5": md5
                                        }, success: function () {
    ...
        
    <script type="text/javascript">
        var vipmaxsize = 500 * 1024 * 1024;
        var normalmaxsize = 200 * 1024 * 1024;
        var viperrMsg = "VIP用户上传的附件文件不能超过500M！！！";
        var normalerrMsg = "普通用户上传的附件文件不能超过2000M！！！";
